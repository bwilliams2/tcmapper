...
def populate_elections():
    ...
    try:
        ...
        for n, file in enumerate(files):
            ...
            connection.commit()
        for year in years:
            cursor.execute(f"UPDATE parcels SET vtdid_{year} = a.vtdid FROM election a WHERE a.year = {year} AND ST_Intersects(a.geom::geometry, parcels.geom_c::geometry)")
        ...
        if table_exists:
            ...
            for n, rec in tqdm(enumerate(recs), total=len(recs)):
                ...
                if n % 100:
                    connection.commit()
            connection.commit()
        cursor.execute(f"UPDATE parcels SET grid_id = a.grid_id FROM grids a WHERE ST_Intersects(a.geom::geometry, parcels.geom_c::geometry)")
